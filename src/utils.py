...
def gen_report(report_path:Path):

    """
    - Generate PNG hetmap files for observation
    - generate a table of N best params
    """

    report_df = load_gsreport(report_path)
    filename = report_path
    # filename = str(report_path).split('/')[-1]
    ...
    ### plot biomass / harvest curves if available ###
    bc_file = str(report_path) + '.biomass_df.csv'
    hv_file = str(report_path) + '.harvest_df.csv'

    if Path(DATA_PATH / bc_file).exists() and Path(DATA_PATH / hv_file).exists():
        plot_curves(
            biomass_path=DATA_PATH / bc_file,
            harvest_path=DATA_PATH / hv_file,
            fname=filename
        )
